...
public class ForkJoinPool extends AbstractExecutorService {
    ...
    /**
     * 支持工作窃取和外部任务提交的队列。有关描述和算法，请参见上文。
     * 大多数平台上的性能对 WorkQueue 及其数组的实例放置非常敏感——
     * 我们绝对不希望多个 WorkQueue 实例或多个队列数组共享缓存行
     */
    @sun.misc.Contended
    static final class WorkQueue {
        ...
        /**
         * 如果存在，则从队列中删除并执行给定任务或任何其他已取消的任务。仅由 awaitJoin 使用
         *
        ...
        /**
         * 如果在与给定任务相同的 CC 计算中，则以共享或拥有模式弹出任务。仅由 helpComplete 使用
         */
        ...
        /**
         * 窃取并运行与给定任务相同的 CC 计算中的任务（如果存在并且可以在没有争用的情况下获取）。
         * 否则返回校验和/控制值以供方法 helpComplete 使用
         *
    ...
    // 配置：二进制的低16位代表 并行度（parallelism），高16位：mode可选FIFO_QUEUE（1 << 16）和LIFO_QUEUE（1 << 31），默认是LIFO_QUEUE
    ...
    /**
     * 尝试在目标的计算中窃取和运行任务。使用顶级算法的变体，仅限于以给定任务为祖先的任务：
     * 它更喜欢获取并运行从工作人员自己的队列中弹出的合格任务（通过 popCC）。
     * 否则它会扫描其他人，随机进行争用或执行，根据校验和决定放弃（通过返回码 frob pollAndExecCC）。
     * maxTasks 参数支持外部使用；内部调用使用零，允许无限制的步骤（外部调用捕获非正值）
     *
     * @param w        caller
     * @param maxTasks 如果非零，则要运行的其他任务的最大数量
    ...
    final int helpComplete(WorkQueue w, CountedCompleter<?> task, int maxTasks) {
        ...
        if ((ws = workQueues) != null && (m = ws.length - 1) >= 0 && task != null && w != null) {
            ...
            // 第一个扫描队列
            int origin = r & m;
            // 1:ran == 直接运行, >1:contended == 竞争, <0:hash == 哈希
            int h = 1;
    ...
    /**
     * 尝试为给定任务的窃取者定位和执行任务，或者依次为它的窃取者之一，
     * 跟踪 currentSteal -> currentJoin 链接，寻找在给定任务的后代上工作的线程，并使用非空队列窃取回并从中执行任务。
     * 在等待加入时第一次调用此方法通常需要扫描搜索（这没关系，因为加入者无事可做），但此方法会在工作人员中留下提示以加快后续调用.
     *
     * @param w    caller
    ...
    /**
     * externalPush 的完整版，处理不常见的情况，以及在第一次提交
     * 第一个任务到池时执行二次初始化。它还检测外部线程的第一次提交，
     * 如果索引处的队列为空或竞争，则创建一个新的共享队列
     *
     * @param task the task. Caller must ensure non-null.
     */
