...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    /**
     * 空闲线程等待工作的超时时间（以纳秒为单位）。当存在多个corePoolSize或allowCoreThreadTimeOut时，
     * 线程将使用此超时。否则，他们将永远等着新工作.
     */
    ...
    private final class Worker extends AbstractQueuedSynchronizer implements Runnable {
        ...
        /**
         * 使用给定的firstTask和ThreadFactory中的线程进行创建.
         *
         * @param firstTask the first task (null if none)
         */
        Worker(Runnable firstTask) {
            // 将state值设置为-1，原因是防止线程被中断
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (; ; ) {
            ...
            // 根据线程池状态以及阻塞队列是否为空
            if (rs >= SHUTDOWN &&
                    // 线程池处于关闭态，并且添加的任务为空，并且阻塞队列非空同时成立
        ...
        try {
            ...
            if (t != null) {
                ...
                mainLock.lock();
                try {
                    // 按住锁时重新检查。 退出ThreadFactory故障，或者如果在获得锁之前关闭了.
                    int rs = runStateOf(ctl.get());
    ...
    /**
     * 为垂死的工人进行清理和簿记。仅在工作线程中称为*。除非设置了completelyAbruptly，
     * 否则假定workerCount已被调整为帐户可以退出。此方法从工作程序集中删除线程，
     * 并且如果它由于用户任务异常而退出，或者少于 corePoolSize工作程序正在运行或队列
     * 非空但没有工作程序，则可能终止该池或替换工作程序。
     *
     * @param w                 the worker
    ...
    private Runnable getTask() {
        ...
        for (; ; ) {
            ...
            // 如果线程池处于STOP、TIDYING、TERMINATED三种状态之一
            // 或者处于SHUTDOWN状态且阻塞队列为空时，执行decrementWorkerCount()
            ...
            // 1、是否允许核心线程过期；2、当前运行线程个数是否大于核心线程阈值。
            boolean timed = allowCoreThreadTimeOut || wc > corePoolSize;
            // 首先根据是否执行过期策略(timed)，以不同的方式(poll和take)从阻塞队列获取Runnable，
            // 如果获取任务r非空，则返回r。如果获取任务为空，则说明已经到了过期时间，
            // timedOut = true，那么在下一次循环时，线程数减一，退出方法
    ...
    final void runWorker(Worker w) {
        ...
        try {
            // 当前任务不为空或者队列中任务不为空（阻塞队列）
            while (task != null || (task = getTask()) != null) {
                w.lock();
                // 如果池正在停止，请确保线程被中断；如果没有，请确保线程不被中断。
                // 这需要在第二种情况下进行重新检查以处理shutdownNow竞赛，同时清除中断
     ...
     *  <p> 如果由于该执行程序已关闭或已达到其容量而无法提交执行任务，
     *  则该任务由当前执行 {@code RejectedExecutionHandler}.
     *
    ...
    public void execute(Runnable command) {
         ...
         * Proceed in 3 steps:
         *
         * 1. 如果正在运行的线程少于corePoolSize线程，请尝试使用给定命令作为其第一个任务
         * 启动新线程。对addWorker的调用原子地检查runState和workerCount,
         * 从而通过返回false来防止在不应该添加线程的情况下发出虚假警报。
         *
         * 2. 如果一个任务可以成功地排队，那么我们仍然需要再次检查是否应该添加一个线程
         * （因为现有线程自上次检查后就死了），或者自进入该方法以来该池已关闭。因此，我们
         * 重新检查状态，并在必要时回滚排队，如果已停止，或者如果没有线程则启动一个新线程.
         *
         * 3. 如果我们无法将任务排队，则尝试添加一个新的线程。
         * 如果失败，则表明我们已关闭或处于饱和状态，因此拒绝了该任务.
         */
         ...
         * 5.如果workerCount >= maximumPoolSize，并且线程池内的阻塞队列已满,
