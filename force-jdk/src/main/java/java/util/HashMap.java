...
public class HashMap<K, V> extends AbstractMap<K, V>
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict) {
        ...
        if ((tab = table) == null || (n = tab.length) == 0)
            n = (tab = resize()).length;
        // 没有出现hash冲突，直接newNode存入数组
        if ((p = tab[i = (n - 1) & hash]) == null)
            // 当多个线程进行put同一个key时，先put的value会被覆盖
            ...
            if (p.hash == hash &&
                ...
                for (int binCount = 0; ; ++binCount) {
                    ...
                    if ((e = p.next) == null) {
                        ...
                        // binCount >= 7 转换为红黑树（数组大于64才会进行，否则resize）
                        if (binCount >= TREEIFY_THRESHOLD - 1) // -1 for 1st
                            treeifyBin(tab, hash);
                        break;
                    }
                    // 目标节点与当前节点相同，则退出循环不进行覆盖
                    if (e.hash == hash &&
                            ((k = e.key) == key || (key != null && key.equals(k))))
                        break;
                    p = e;
            ...
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    ...
                    if (e.next == null)
                        ...
                        // 因此不进行详解，当树的个数小于等于UNTREEIFY_THRESHOLD则转成链表
    ...
    /**
     * 为给定的哈希替换索引处bin中所有链接的节点，除非表太小，在这种情况下将调整大小.
     */
    final void treeifyBin(Node<K, V>[] tab, int hash) {
        int n, index;
        Node<K, V> e;
        // 数组长度小于64进行resize不将链表为转换红黑树
        if (tab == null || (n = tab.length) < MIN_TREEIFY_CAPACITY)
            resize();
            // 只有table长度大于64时才会将链表节点转换成红黑树
